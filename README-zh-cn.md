...
2. **流量配额与超售管理 (Oversell)**：
   - **严格模式（禁止超售）**：配额限制分销商名下所有用户分配的额度总和 (`∑ user.data_limit ≤ traffic_limit`)，无法创建无限流量用户或超额分配。
   - **超售模式**：配额限制实际已消耗的总流量 (`∑ user.used_traffic + 历史重置 ≤ traffic_limit`)，可自由分配套餐，直到实际消耗达到配额上限。
