...
2. **Квоты трафика и Оверселлинг**:
   - **Строгий режим (без оверселлинга)**: Квота ограничивает сумму лимитов пользователей (`∑ user.data_limit ≤ traffic_limit`). Реселлер не может перераспределить лимит или создать безлимитные аккаунты.
   - **Режим оверселлинга**: Квота ограничивает реально потребленный трафик пользователей (`∑ user.used_traffic + сбросы ≤ traffic_limit`).
